    ...
    <div class="card-body">
        <form action="{{ route("admin.products.update", [$product->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                <label for="code">{{ trans('global.product.fields.code') }}*</label>
                <input type="text" id="code" name="code" class="form-control" value="{{ old('code', isset($product) ? $product->code : '') }}">
                @if($errors->has('code'))
                    <em class="invalid-feedback">
                        {{ $errors->first('code') }}
                    </em>
                @endif
                <p class="helper-block">
                    {{ trans('global.product.fields.code_helper') }}
                </p>
            </div>
            
            <div class="form-group {{ $errors->has('count') ? 'has-error' : '' }}">
                <label for="code">{{ trans('global.product.fields.count') }}*</label>
                <input type="text" id="count" name="count" class="form-control" value="{{ old('count', isset($product) ? $product->count : '') }}">
                @if($errors->has('count'))
                    <em class="invalid-feedback">
                        {{ $errors->first('count') }}
                    </em>
                @endif
                <p class="helper-block">
                    {{ trans('global.product.fields.count_helper') }}
                </p>
            </div>
